...
install_realm() {
    if [ -f "/usr/local/bin/realm" ]; then
        echo -e "${YELLOW}Realm 已安装，重新安装将覆盖现有配置${PLAIN}"
    ...
    echo -e "${YELLOW}正在下载 Realm...${PLAIN}"
    mkdir -p /tmp/realm
...
EOF
    ...
    echo -e "使用 ${YELLOW}realm-manager${PLAIN} 命令管理"
}
...
show_status() {
    ...
    if [ -f "/usr/local/bin/realm" ]; then
        ...
        echo -e "  ${CYAN}Realm 版本: ${YELLOW}${INSTALLED_VERSION:-未知}${PLAIN}"
    else
